...
export const ProductSections = () => {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    // Brand filter strictly applies ONLY when viewing 'ALL' categories or 'Mobiles & Smartphones'
    const cleanSelCat = (selectedCategory || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    const isMobileOrAllCat = !selectedCategory || selectedCategory === 'ALL' || cleanSelCat === 'all' || cleanSelCat === 'mobilessmartphones';

    if (isMobileOrAllCat && selectedBrand && selectedBrand !== 'ALL') {
